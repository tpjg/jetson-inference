...
// gpuPreImageNetMeanChar - same as gpuPreImageNetMean but taking uchar4 input, coverting to float3 while resizing and subtracting mean
__global__ void gpuPreImageNetMeanChar( float2 scale, uchar4* input, int iWidth, float* output, int oWidth, int oHeight, float3 mean_value )
{
	const int x = blockIdx.x * blockDim.x + threadIdx.x;
	const int y = blockIdx.y * blockDim.y + threadIdx.y;
	const int n = oWidth * oHeight;
	...
	const uchar4 px  = input[ dy * iWidth + dx ];
	const float3 bgr = make_float3(px.z - mean_value.x, px.y - mean_value.y, px.x - mean_value.z);
...
// cudaPreImageNetMean - same as cudaPreImageNetMean but taking uchar4 input, coverting to float3 while resizing and subtracting mean
cudaError_t cudaPreImageNetMeanChar( uchar4* input, size_t inputWidth, size_t inputHeight,
				             float* output, size_t outputWidth, size_t outputHeight, const float3& mean_value )
{
	if( !input || !output )
		return cudaErrorInvalidDevicePointer;
	...
	gpuPreImageNetMeanChar<<<gridDim, blockDim>>>(scale, input, inputWidth, output, outputWidth, outputHeight, mean_value);
